...
class ContextEncoder(torch.nn.Module):
    ...
    def forward(self, img):
        ...
        # Store all tokens that are masked out in a mask. The mask it True for every element that is masked
        mask = (torch.norm(patches, p=2, dim=-1) > 0)  # shape (B, N)
        ...
        # Pass mask to transformer blocks for attention masking
        for block in self.transformer_blocks:
            tokens = block(tokens, key_padding_mask=mask)

        # Return tokens
        return self.output_layer_norm(tokens), mask